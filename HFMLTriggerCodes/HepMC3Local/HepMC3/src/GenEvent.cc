...
GenEvent::GenEvent(Units2::MomentumUnit mu,
                   Units2::LengthUnit lu)
...
void GenEvent::set_units(Units2::MomentumUnit new_momentum_unit, Units2::LengthUnit new_length_unit) {
    if ( new_momentum_unit != m_momentum_unit ) {
        for ( GenParticlePtr& p: m_particles ) {
            Units2::convert(p->m_data.momentum, m_momentum_unit, new_momentum_unit);
            Units2::convert(p->m_data.mass, m_momentum_unit, new_momentum_unit);
        }
    ...
    if ( new_length_unit != m_length_unit ) {
        for (GenVertexPtr& v: m_vertices) {
            FourVector2 &fv = v->m_data.position;
            if ( !fv.is_zero() ) Units2::convert( fv, m_length_unit, new_length_unit );
        }
...
const FourVector2& GenEvent::event_pos() const {
...
void GenEvent::shift_position_by(const FourVector2 & delta) {
...
bool GenEvent::rotate(const FourVector2&  delta)
{
    for ( auto& p: m_particles)
    {
        const FourVector2& mom = p->momentum();
        ...
        FourVector2 temp(tempX, tempY, tempZ, mom.e());
        p->set_momentum(temp);
    }
    for (auto& v: m_vertices)
    {
        const FourVector2& pos = v->position();
        ...
        FourVector2 temp(tempX, tempY, tempZ, pos.t());
        v->set_position(temp);
    }
...
bool GenEvent::reflect(const int axis)
{
    ...
    {
    case 0:
        for ( auto& p: m_particles) { FourVector2 temp = p->momentum(); temp.setX(-p->momentum().x()); p->set_momentum(temp);}
        for ( auto& v: m_vertices)  { FourVector2 temp = v->position(); temp.setX(-v->position().x()); v->set_position(temp);}
        break;
    case 1:
        for ( auto& p: m_particles) { FourVector2 temp = p->momentum(); temp.setY(-p->momentum().y()); p->set_momentum(temp);}
        for ( auto& v: m_vertices)  { FourVector2 temp = v->position(); temp.setY(-v->position().y()); v->set_position(temp);}
        break;
    case 2:
        for ( auto& p: m_particles) { FourVector2 temp = p->momentum(); temp.setZ(-p->momentum().z()); p->set_momentum(temp);}
        for ( auto& v: m_vertices)  { FourVector2 temp = v->position(); temp.setZ(-v->position().z()); v->set_position(temp);}
        break;
    case 3:
        for ( auto& p: m_particles) { FourVector2 temp = p->momentum(); temp.setT(-p->momentum().e()); p->set_momentum(temp);}
        for ( auto& v: m_vertices)  { FourVector2 temp = v->position(); temp.setT(-v->position().t()); v->set_position(temp);}
        break;
    default:
        return false;
    }
...
bool GenEvent::boost(const FourVector2&  delta)
{
    ...
    for ( auto& p: m_particles)
    {
        ...
        FourVector2 temp(tempX, tempY, tempZ, tempE);
        p->set_momentum(temp);
    }
