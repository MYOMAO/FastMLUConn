...
bool ReaderAsciiHepMC2::parse_units(GenEvent &evt, const char *buf) {
    ...
    ++cursor;
    Units2::MomentumUnit momentum_unit = Units2::momentum_unit(cursor);

    // length
    if ( !(cursor = strchr(cursor+1, ' ')) ) return false;
    ++cursor;
    Units2::LengthUnit length_unit = Units2::length_unit(cursor);

    evt.set_units(momentum_unit, length_unit);

    HEPMC3_DEBUG(10, "ReaderAsciiHepMC2: U: " << Units2::name(evt.momentum_unit()) << " " << Units2::name(evt.length_unit()))
...
int ReaderAsciiHepMC2::parse_vertex_information(const char *buf) {
    ...
    data->set_position(FourVector2(X,Y,Z,T));
...
int ReaderAsciiHepMC2::parse_particle_information(const char *buf) {
    ...
    data->set_momentum(FourVector2(Px,Py,Pz,E));
