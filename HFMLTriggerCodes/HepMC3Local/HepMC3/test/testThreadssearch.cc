...
std::shared_ptr<GenEvent> generate(const int Z) {
    ...
    GenParticlePtr b2 = std::make_shared<GenParticle>( FourVector2( 0.0,    0.0,   7000.0,  7000.0  ),2212,  3 );
    GenParticlePtr b1 = std::make_shared<GenParticle>( FourVector2( 0.750, -1.569,   32.191,  32.238),   1,  3 );
    ...
    for (int z = 0; z < Z; z++) {
        ...
        for (auto p: particles) {
            ...
            GenParticlePtr p2 = std::make_shared<GenParticle>( FourVector2( 0.0,    0.0,   7000.0+0.01*evt->particles().size(),  7000.0  ),2212,  3 );
            GenParticlePtr p1 = std::make_shared<GenParticle>( FourVector2( 0.750, -1.569,   32.191+0.01*evt->particles().size(),  32.238),   1,  3 );
