...
public:
...
#endif
    {
        ...
        evt->set_units(Units2::GEV,Units2::MM);
        ...
        for(int i=0; i<pyev.size(); ++i) {
            hepevt_particles.push_back( std::make_shared<GenParticle>( FourVector2( pyev[i].px(), pyev[i].py(),
                                        pyev[i].pz(), pyev[i].e() ),
        ...
        for(int  i=0; i<pyev.size(); ++i) {
            ...
            if(mothers.size()) {
                ...
                FourVector2 prod_pos( pyev[i].xProd(), pyev[i].yProd(),pyev[i].zProd(), pyev[i].tProd() );
