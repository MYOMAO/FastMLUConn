...
public:
    ...
    GenEvent(Units2::MomentumUnit mu = Units2::GEV,
             Units2::LengthUnit lu = Units2::MM);

#if !defined(__CINT__)

    /// @brief Constructor with associated run
    GenEvent(std::shared_ptr<GenRunInfo> run,
             Units2::MomentumUnit mu = Units2::GEV,
             Units2::LengthUnit lu = Units2::MM);
    ...
    const Units2::MomentumUnit& momentum_unit() const { return m_momentum_unit; }
    /// @brief Get length unit
    const Units2::LengthUnit& length_unit() const { return m_length_unit; }
    /// @brief Change event units
    /// Converts event from current units to new ones
    void set_units( Units2::MomentumUnit new_momentum_unit, Units2::LengthUnit new_length_unit);
    ...
    const FourVector2& event_pos() const;
...
private:
    ...
    /// Momentum unit
    Units2::MomentumUnit m_momentum_unit = Units2::GEV;
    /// Length unit
    Units2::LengthUnit m_length_unit = Units2::MM;
