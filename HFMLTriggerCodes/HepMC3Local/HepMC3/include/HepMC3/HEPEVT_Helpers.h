...
bool HEPEVT_to_GenEvent_nonstatic(GenEvent* evt, T* A)
{
    ...
    for ( int i = 1; i <= ne; i++ )
    {
        ...
        p->set_momentum(FourVector2(A->px(i), A->py(i), A->pz(i), A->e(i)));
        ...
        v->set_position(FourVector2(A->x(i), A->y(i), A->z(i), A->t(i)));
...
bool GenEvent_to_HEPEVT_nonstatic(const GenEvent* evt, T* A)
{
    ...
    for ( int i = 1; i <= particle_counter; ++i )
    {
        ...
        FourVector2 m = sorted_particles[i-1]->momentum();
        ...
        {
            FourVector2 p = sorted_particles[i-1]->production_vertex()->position();
...
bool HEPEVT_to_GenEvent_static(GenEvent* evt)
{
    ...
    {
        GenParticlePtr p = std::make_shared<GenParticle>();
        p->set_momentum(FourVector2(T::px(i), T::py(i), T::pz(i), T::e(i)));
        ...
        particles_index[i] = p;
        GenVertexPtr v = std::make_shared<GenVertex>();
        v->set_position(FourVector2(T::x(i), T::y(i), T::z(i), T::t(i)));
        v->add_particle_out(p);
        std::set<int> in;
        std::set<int> out;
        out.insert(i);
