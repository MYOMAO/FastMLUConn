...
/**
 *  @file FourVector2.h
 *  @brief Definition of \b class FourVector2
 */
#include <cmath>
#include <limits>
#ifndef M_PI
...
public:
    ...
    /** @brief Sets all FourVector2 fields */
    FourVector2(double xx, double yy, double zz, double ee)
        : m_v1(xx),     m_v2(yy),   m_v3(zz),      m_v4(ee)    {}
    /** @brief Copy constructor */
    FourVector2(const FourVector2 &) = default;
    /** @brief Move constructor */
    FourVector2(FourVector2 && ) = default;
    /** @brief = */
    FourVector2& operator=(const FourVector2&) = default;
    /** @brief = */
//    FourVector2& operator=(FourVector2FourVector2&&FourVector&&FourVector2&&FourVector&&FourVector2&&FourVector&&FourVector2&&FourVector&&) = default;
    FourVector2& operator=(FourVector2&&) = default;
    ...
    /** @brief Set all FourVector2 fields, in order x,y,z,t */
    ...
    /// @name Comparisons to another FourVector2
    /// @{
    ...
    double delta_phi(const FourVector2 &v) const {
    ...
    /// Equality
    bool operator==(const FourVector2& rhs) const {
        return x() == rhs.x() && y() == rhs.y() && z() == rhs.z() && t() == rhs.t();
    }
    /// Inequality
    bool operator!=(const FourVector2& rhs) const { return !(*this == rhs); }

    /// Arithmetic operator +
    FourVector2  operator+ (const FourVector2& rhs) const {
        return FourVector2( x() + rhs.x(), y() + rhs.y(), z() + rhs.z(), t() + rhs.t() );
    }
    /// Arithmetic operator -
    FourVector2  operator- (const FourVector2& rhs) const {
        return FourVector2( x() - rhs.x(), y() - rhs.y(), z() - rhs.z(), t() - rhs.t() );
    ...
    FourVector2  operator* (const double rhs) const {
        return FourVector2( x()*rhs, y()*rhs, z()*rhs, t()*rhs );
    }
    /// Arithmetic operator / by scalar
    FourVector2  operator/ (const double rhs) const {
        return FourVector2( x()/rhs, y()/rhs, z()/rhs, t()/rhs );
    ...
    /// Arithmetic operator +=
    void operator += (const FourVector2& rhs) {
        setX(x() + rhs.x());
        setY(y() + rhs.y());
        setZ(z() + rhs.z());
        setT(t() + rhs.t());
    ...
    /// Static null FourVector2 = (0,0,0,0)
    static const FourVector2& ZERO_VECTOR() {
        static const FourVector2 v;
        return v;
    }
...
inline double delta_phi(const FourVector2 &a, const FourVector2 &b) { return b.delta_phi(a); }

/// Pseudorapidity separation between vecs @c a and @c b
inline double delta_eta(const FourVector2 &a, const FourVector2 &b) { return b.delta_eta(a); }

/// Rapidity separation between vecs @c a and @c b
inline double delta_rap(const FourVector2 &a, const FourVector2 &b) { return b.delta_rap(a); }

/// R_eta^2-distance separation dR^2 = dphi^2 + deta^2 between vecs @c a and @c b
inline double delta_r2_eta(const FourVector2 &a, const FourVector2 &b) { return b.delta_r2_eta(a); }

/// R_eta-distance separation dR = sqrt(dphi^2 + deta^2) between vecs @c a and @c b
inline double delta_r_eta(const FourVector2 &a, const FourVector2 &b) { return b.delta_r_eta(a); }

/// R_rap^2-distance separation dR^2 = dphi^2 + drap^2 between vecs @c a and @c b
inline double delta_r2_rap(const FourVector2 &a, const FourVector2 &b) { return b.delta_r2_rap(a); }

/// R_rap-distance separation dR = sqrt(dphi^2 + drap^2) between vecs @c a and @c b
inline double delta_r_rap(const FourVector2 &a, const FourVector2 &b) { return b.delta_r_rap(a); }
