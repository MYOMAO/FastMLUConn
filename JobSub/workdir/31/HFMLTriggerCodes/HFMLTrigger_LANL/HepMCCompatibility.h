...
HepMC::GenEvent* ConvertHepMCGenEvent_3to2(const HepMC3::GenEvent& evt)
{
    ...
    if (evt.length_unit()==HepMC3::Units2::CM) lu=HepMC::Units::CM;
    if (evt.length_unit()==HepMC3::Units2::MM) lu=HepMC::Units::MM;
    if (evt.momentum_unit()==HepMC3::Units2::MEV) mu=HepMC::Units::MEV;
    if (evt.momentum_unit()==HepMC3::Units2::GEV) mu=HepMC::Units::GEV;
    n->use_units(mu,lu);
    ...
    {
        HepMC3::FourVector2 pos3=v3->position();
    ...
    {
        HepMC3::FourVector2 mom3=p3->momentum();
...
HepMC3::GenEvent*  ConvertHepMCGenEvent_2to3(const HepMC::GenEvent& evt, std::shared_ptr<HepMC3::GenRunInfo> run )
{
	...
	std::cout << "Pass Haha - mY bug" << std::endl;

    HepMC3::Units2::LengthUnit lu;
    HepMC3::Units2::MomentumUnit mu;
	std::cout << "No Fuckin BUGS" << std::endl;
	
    if (evt.length_unit()==HepMC::Units::CM) lu=HepMC3::Units2::CM;
    if (evt.length_unit()==HepMC::Units::MM) lu=HepMC3::Units2::MM;
    if (evt.momentum_unit()==HepMC::Units::MEV) mu=HepMC3::Units2::MEV;
    if (evt.momentum_unit()==HepMC::Units::GEV) mu=HepMC3::Units2::GEV;
    n->set_units(mu,lu);
    ...
    for (auto v2=evt.vertices_begin(); v2!=evt.vertices_end(); v2++)
    {
        ...
        HepMC3::FourVector2 pos3=HepMC3::FourVector2(pos2.x(),pos2.y(),pos2.pz(),pos2.t());
    ...
    for (auto p2=evt.particles_begin(); p2!=evt.particles_end(); p2++)
    {
        ...
        HepMC3::FourVector2 mom3=HepMC3::FourVector2(mom2.px(),mom2.py(),mom2.pz(),mom2.e());
